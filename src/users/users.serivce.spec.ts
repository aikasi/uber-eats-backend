...
describe('UserService', () => {
  ...
  describe('login', () => {
    ...
    it('should fail if the password is wrong', async () => {
      ...
      usersRepository.findOne.mockResolvedValue(mockedUsr);
      const result = await service.login(loginArgs);
      expect(result).toEqual({ ok: false, error: 'wrong passdword' });
    });

    it('should return token if assword correct', async () => {
      const mockedUsr = {
        id: 1,
